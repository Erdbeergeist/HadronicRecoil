...
int main(int argc, char *argv[]) {
	...
	string addinfile = argv[2];
	const char* outfile = argv[3];
			
	TFile *fileI = new TFile(infile.c_str(),"READ"); /// Open Input ROOT File
	if (fileI->IsOpen())	cout<<"File "<<infile<<" opened successfully.\n";
	
	TFile *fileAI = new TFile(addinfile.c_str(),"READ"); /// Open Input ROOT File
	if (fileAI->IsOpen())	cout<<"File "<<addinfile<<" opened successfully.\n";
	...
	for (int i=0;i<totalEvents;i++)	{
		...
		if (isZ == true) {
			...
			hadvec= calcHadronicRecoilnoasso(vecCellsPt,vecCellsEta,vecCellsPhi,countPVvec,countSVvec,SumPtPVvec,SumPtSVvec,mu1,mu2,NumberOfVertices,all_track_pt,all_track_eta,all_track_phi,fileAI);
			//TVector3 HadronicRecoil2 = calcHadronicRecoilPrimVertnoasso(vecCellsPt,vecCellsEta,vecCellsPhi,countPVvec,countSVvec,mu1,mu2,NumberOfVertices,all_track_pt,all_track_eta,all_track_phi,fileAI);
			//TVector3 HadronicRecoil3 = calcHadronicRecoilPrimVertScalednoasso(vecCellsPt,vecCellsEta,vecCellsPhi,countPVvec,countSVvec,mu1,mu2,NumberOfVertices,all_track_pt,all_track_eta,all_track_phi,fileAI);
			//TVector3 HadronicRecoil4 = calcHadronicRecoilPrimVertPtScalednoasso(vecCellsPt,vecCellsEta,vecCellsPhi,countPVvec,countSVvec,SumPtPVvec,SumPtSVvec,mu1,mu2,NumberOfVertices,all_track_pt,all_track_eta,all_track_phi,fileAI);
			...
			hists.FillHists(hadvec[0],Zvec);
			hists2.FillHists(hadvec[1],Zvec);
			hists3.FillHists(hadvec[2],Zvec);
			hists4.FillHists(hadvec[3],Zvec);
			//hists5.FillHists(HadronicRecoil5,Zvec);
			//hists6.FillHists(HadronicRecoil6,Zvec);
			//hists7.FillHists(HadronicRecoil7,Zvec);
			hists.Fill2DHists(hadvec[0],Zvec,NumberOfVertices);
			hists2.Fill2DHists(hadvec[1],Zvec,NumberOfVertices);
			hists3.Fill2DHists(hadvec[2],Zvec,NumberOfVertices);
			hists4.Fill2DHists(hadvec[3],Zvec,NumberOfVertices);
