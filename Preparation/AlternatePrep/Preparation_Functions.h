	...
	for (int i =0;i<event->trk_eta_atCalo2ndLayer->size();i++){
		...
		for (int j = 0; j<vecCaloCells.size();j++){
			if ( sqrt ( pow(tTrack.Eta()-vecCaloCells[j].vecCell.Eta(),2) + pow(tTrack.Phi()-vecCaloCells[j].vecCell.Phi(),2) ) < 0.1 ){
				if(isPrim==true){
					vecCaloCells[j].primtracks.push_back(tTrack);
					vecCaloCells[j].numberOfAssociatedPrimaryTracks+=1;
					vecCaloCells[j].sumPtOfAssociatedPrimaryTracks+=tTrack.Pt();
				}	
				else if(isPrim==false){
					vecCaloCells[j].pileuptracks.push_back(tTrack);
					vecCaloCells[j].numberOfAssociatedPileUpVertexTracks+=1;
					vecCaloCells[j].sumPtOfAssociatedPileUpVertexTracks+=tTrack.Pt();
				}
				associated = true;	
			}
		}
